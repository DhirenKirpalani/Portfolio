...
export default function Experience() {
  ...
  return (
    <section id="experience" className="py-16 sm:py-24 md:py-32 relative overflow-hidden" style={{ background: 'var(--background-secondary)' }}>
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        >
          ...
          <motion.div 
            ...
            className="h-1 bg-gradient-to-r from-blue-500 via-purple-500 to-pink-500 mb-10 sm:mb-16 md:mb-20 rounded-full" 
          />
            ...
            <div className="space-y-10 sm:space-y-12 md:space-y-16">
              {experiences.map((exp, index) => (
                <motion.div
                  ...
                  className="relative pl-6 sm:pl-8 md:pl-24"
                >
                  <motion.div 
                    initial={{ scale: 0 }}
                    animate={isInView ? { scale: 1 } : { scale: 0 }}
                    transition={{ delay: index * 0.2 + 0.3 }}
                    className="absolute left-0 md:left-8 top-2 w-3 h-3 sm:w-4 sm:h-4 bg-gradient-to-r from-blue-500 to-purple-500 rounded-full border-4 border-[#050505] transform -translate-x-[5.5px] sm:-translate-x-[7.5px] shadow-lg shadow-blue-500/50" 
                  />
                  ...
                  >
                    ...
                    <div className="flex items-start justify-between mb-4 sm:mb-6 flex-wrap gap-3 sm:gap-4">
                      <div>
                        <h3 className={`text-xl sm:text-2xl md:text-3xl font-bold mb-2 sm:mb-3 group-hover:text-blue-500 transition-colors leading-tight ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>
                          {exp.role}
                        </h3>
                        <div className={`flex items-center gap-2 ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>
                          <Briefcase className="w-4 h-4 sm:w-5 sm:h-5" />
                          <span className="font-medium text-base sm:text-lg">{exp.company}</span>
                        </div>
                      </div>
                      <span className="text-xs sm:text-sm text-white font-semibold bg-gradient-to-r from-blue-500 to-purple-500 px-3 sm:px-5 py-2 sm:py-2.5 rounded-full shadow-lg">
                        {exp.period}
                      </span>
                    </div>
                    ...
                    <ul className="space-y-3 sm:space-y-4">
                      {exp.highlights.map((highlight, i) => (
                        <li key={i} className={`flex items-start gap-3 sm:gap-4 text-sm sm:text-base md:text-lg leading-relaxed ${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>
